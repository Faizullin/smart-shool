...
export class SubjectGroupAssignComponent {
  public table_title = 'Subject Groups Assign';
  public action_urls = {
  ...
  constructor(
    ...
    // super(http, modalService, authStorageService);
  }
  ngOnInit() {
  ...
  protected fetchData() {
    ...
    this.http.get(this.action_urls.list()).subscribe({
      next: (data: any) => {
  ...
  public onResultEditOpen(item: Result) {
    ...
    this.modalService.show(ResultEditComponent, {
      initialState,
      class: 'modal-lg',
    });
  }
